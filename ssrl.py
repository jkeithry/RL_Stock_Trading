""" 
Single Stock Reinforcement Learning (SSRL) 
Code modified from the following tutorial by FinRL: http://finrl.org/examples/multiplestocktrading.html
"""
######## IMPORTS ########
import pandas as pd
import numpy as np
...
######## HELPER FUNCTIONS ########
# downloadData('AAPL', '2009-01-01', '2020-10-23')
...
def add_technical_indicator(df):
    ...
    ## macd
    macd = stock[stock.tic == unique_ticker[0]]['macd']
    ## rsi
    rsi = stock[stock.tic == unique_ticker[0]]['rsi_30']
        
    df['macd'] = macd
    df['rsi'] = rsi
    return df.rename(columns={"Adj Close": "adjcp", "Close": "close"}) # renamed to match environment setup
...
def test_model(model, data, initial_balance):
    print("!-------- Testing Model --------!")
    env = StockEnvTrain(df=data, initial_balance=initial_balance)
    ...
    print("Finished!")
    # print("Reward earned: {}".format(total_rewards))
...
class StockEnvTrain(gym.Env):
    ...
    def __init__(self, df, initial_balance = INITIAL_ACCOUNT_BALANCE, initial_stock = 0, day = 0):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            print("total_reward:{}".format(self.state[0]+sum(np.array(self.state[1:(STOCK_DIM+1)])*np.array(self.state[(STOCK_DIM+1):(STOCK_DIM*2+1)]))- self.initial_balance ))
            ...
            print("Sharpe: ",sharpe)
            print("=================================")
            print("Buys: {}, Sells: {}".format(self.buys,self.sells))
            df_rewards = pd.DataFrame(self.rewards_memory)
            #df_rewards.to_csv('account_rewards_train.csv')
            # print(df_total_value.head(5))
...
class StockTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.data = self.df.iloc[self.day,:]    
            if self.turbulence_threshold is not None:     
                self.turbulence = self.data['turbulence'].values[0] if len(self.df.tic.unique())>1 else self.data['turbulence']
